...
function deviceMotion(e) {
  ...
  acY = int(aIG.y);
  acZ = int(aIG.z);
  if (deviceType == PT_Android) {
    //Android と iOS で正負が逆になる
    acX = -acX;
    acY = -acY;
    acZ = -acZ;
  }
...
function fTri(x0, y0, x1, y1, x2, y2, col) {
  //三角（塗り潰し）
  bg.fillStyle = col;
  bg.beginPath();
  bg.moveTo(x0, y0);
...
function sTri(x0, y0, x1, y1, x2, y2, col) {
  //三角（線）
  bg.strokeStyle = col;
  bg.beginPath();
  bg.moveTo(x0, y0);
...
function drawImgC(n, x, y) {
  //センタリング表示
  if (img_loaded[n] == true)
    bg.drawImage(img[n], x - int(img[n].width / 2), y - int(img[n].height / 2));
}

function drawImgS(n, x, y, w, h) {
  //拡大縮小
  if (img_loaded[n] == true) bg.drawImage(img[n], x, y, w, h);
}

function drawImgTS(n, sx, sy, sw, sh, cx, cy, cw, ch) {
  //切り出し＋拡大縮小
  if (img_loaded[n] == true)
    bg.drawImage(img[n], sx, sy, sw, sh, cx, cy, cw, ch);
}

function drawImgR(n, x, y, ang) {
  //回転
  if (img_loaded[n] == true) {
    var w = img[n].width;
    var h = img[n].height;
...
function wwsSysMain() {
  ...
  if (DEBUG) {
    ...
    fText(
      'winW=' + winW + ' winH=' + winH + ' SCALE=' + SCALE,
      x,
      200,
      16,
      'yellow'
    ...
    fText(
      main_idx + ':' + main_tmr + '(' + tapX + ',' + tapY + ')' + tapC,
      x,
      250,
      16,
      'cyan'
